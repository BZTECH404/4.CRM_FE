...
function App() {
  ...
  const handleSave = () => {
    ...
    //   .then((response) => {
    //     console.log(response.data.template._id);
    //   })
    //   .catch((error) => {
    //     console.error(error);
    //   });
    ...
    <div className="App">
      ...
      <div style={{ display: "flex", gap: "20px", padding: "20px" }}>
        ...
        <div>
          ...
          <button onClick={handleAddRow}>Add Row</button>
          <button onClick={(e)=>setshowmodal(!showmodal)}>Save</button> {/* Button to trigger handleSave */}
          {
            showmodal && (
          <>
            <p>Template Name:<input type="text" value={templateName} onChange={(e) => setTemplateName(e.target.value)}/></p>
